...
bool OffsetTunerServer::initialize() {
  ...
  int return_data = packet_handler->write1ByteTxRx(
      port_handler, SUB_CONTROLLER_ID, POWER_CTRL_TABLE, 1);

  if (return_data != 0)
    ROS_ERROR("Torque on DXLs! [%s]",
              packet_handler->getRxPacketError(return_data));
  else
  ...
  ROS_INFO_STREAM("num"
                  ...
                  << ", "
                  << "OffsetAngleRad");
  for (std::map<std::string, JointOffsetData *>::iterator map_it =
    ...
    ROS_INFO_STREAM(i << " | " << joint_name << " : "
                      << joint_data->joint_init_pos_rad_ << ", "
...
void OffsetTunerServer::moveToInitPose() {
  ...
  for (std::map<std::string, JointOffsetData *>::iterator map_it =
    ...
    if (controller_->robot_->dxls_.find(joint_name) ==
        controller_->robot_->dxls_.end()) {
      continue;
    } else {
      offset_init_pose[joint_name] =
          joint_data->joint_init_pos_rad_ + joint_data->joint_offset_rad_;
    }
  }
...
void OffsetTunerServer::commandCallback(const std_msgs::String::ConstPtr &msg) {
  if (msg->data == "save") {
    ...
    for (std::map<std::string, JointOffsetData *>::iterator map_it =
      ...
      offset[joint_name] =
          joint_data->joint_offset_rad_; // edit one of the nodes
      init_pose[joint_name] =
          joint_data->joint_init_pos_rad_; // edit one of the nodes
    }

    yaml_out << YAML::BeginMap;
    yaml_out << YAML::Key << "offset" << YAML::Value << offset;
    yaml_out << YAML::Key << "init_pose_for_offset_tuner" << YAML::Value
             << init_pose;
...
void OffsetTunerServer::jointOffsetDataCallback(
  ...
  // goal position
  ROS_INFO_STREAM(msg->joint_name << " " << msg->goal_value << " "
                                  << msg->offset_value << " " << msg->p_gain
                                  << " " << msg->i_gain << " " << msg->d_gain);
  ...
  uint32_t goal_pose_value =
      controller_->robot_->dxls_[msg->joint_name]->convertRadian2Value(
          goal_pose_rad);
  uint8_t dxl_error = 0;
  uint32_t comm_result = COMM_SUCCESS;
  comm_result =
      controller_->writeCtrlItem(msg->joint_name,
                                 controller_->robot_->dxls_[msg->joint_name]
                                     ->goal_position_item_->item_name_,
  ...
  if (dxl_error != 0) {
    ROS_ERROR_STREAM("goal_pos_set : " << msg->joint_name << "  has error "
                                       << (int)dxl_error);
  }
...
void OffsetTunerServer::jointTorqueOnOffCallback(
  ...
  for (unsigned int i = 0; i < msg->torque_enable_data.size(); i++) {
    ...
    if (map_it == robot_offset_data_.end()) {
      ...
      comm_result =
          controller_->writeCtrlItem(joint_name,
                                     controller_->robot_->dxls_[joint_name]
                                         ->torque_enable_item_->item_name_,
      ...
      if (dxl_error != 0) {
        ROS_ERROR_STREAM("goal_pos_set : " << joint_name << "  has error "
                                           << (int)dxl_error);
      }
    }
  }
...
bool OffsetTunerServer::getPresentJointOffsetDataServiceCallback(
  ...
  for (std::map<std::string, JointOffsetData *>::iterator map_it =
    ...
    humanoid_robot_offset_tuner_msgs::JointOffsetPositionData joint_offset_pos;
    ...
    comm_result =
        controller_->readCtrlItem(joint_name,
                                  controller_->robot_->dxls_[joint_name]
                                      ->present_position_item_->item_name_,
    ...
    if (comm_result != COMM_SUCCESS) {
      ...
      joint_offset_pos.present_value =
          controller_->robot_->dxls_[joint_name]->convertValue2Radian(
              present_pos_value);
...
void OffsetTunerServer::getInitPose(const std::string &path) {
  ...
  if (has_offset_file) {
    ...
    for (YAML::const_iterator node_it = offset_data_node.begin();
         node_it != offset_data_node.end(); node_it++) {
      std::string joint_name = node_it->first.as<std::string>();
      double offset = node_it->second.as<double>();

      std::map<std::string, JointOffsetData *>::iterator robot_offset_data_it =
    ...
    YAML::Node offset_init_pose_node =
        offset_yaml_node[OFFSET_INIT_POS_ROSPARAM_KEY];

    // Initialize Init Pose for Offset Tuning in RobotOffsetData
    for (YAML::const_iterator node_it = offset_init_pose_node.begin();
         node_it != offset_init_pose_node.end(); node_it++) {
      std::string joint_name = node_it->first.as<std::string>();
    ...
    for (std::map<std::string, humanoid_robot_framework::Dynamixel *>::iterator
      ...
      robot_offset_data_[joint_name]->joint_init_pos_rad_ =
