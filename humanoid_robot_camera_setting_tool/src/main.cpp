...
#include "humanoid_robot_camera_setting_tool/humanoid_robot_camera_setting_tool.h"

int main(int argc, char **argv) {
  ros::init(argc, argv, "humanoid_robot_camera_setting_tool");
  ...
  ros::Subscriber camera_param_sub =
      nh.subscribe("/humanoid_robot_camera/set_param", 1, &setCameraParameterCallback);
  ros::Subscriber camera_params_sub =
      nh.subscribe("/humanoid_robot_camera/set_params", 1, &setCameraParametersCallback);

  // web setting
  ros::NodeHandle nh_param("~");
  g_has_path = nh_param.getParam("yaml_path", g_param_path);

  g_param_pub = nh.advertise<humanoid_robot_camera_setting_tool::CameraParams>(
      "/humanoid_robot_camera/camera_params", 1);
  g_param_command_sub =
      nh.subscribe("/humanoid_robot_camera/param_command", 1, &paramCommandCallback);
  g_set_param_client =
      nh.advertiseService("/humanoid_robot_camera/set_camera_params", &setParamCallback);
  g_get_param_client =
      nh.advertiseService("/humanoid_robot_camera/get_camera_params", &getParamCallback);
  g_default_setting_path = ros::package::getPath(ROS_PACKAGE_NAME) +
                           "/config/camera_parameter_default.yaml";

  boost::recursive_mutex config_mutex;

  // dynamic_reconfigure::Server<humanoid_robot_camera_setting_tool::CameraParamsConfig>
  // param_server;
  dynamic_reconfigure::Server<
      humanoid_robot_camera_setting_tool::CameraParamsConfig>::CallbackType callback_fnc;
  g_param_server.reset(
      new dynamic_reconfigure::Server<
          humanoid_robot_camera_setting_tool::CameraParamsConfig>(config_mutex));
...
void dynParamCallback(humanoid_robot_camera_setting_tool::CameraParamsConfig &config,
                      uint32_t level) {
  g_dyn_config = config;
  ...
  // white balance
  setV4lParameter("white_balance_temperature_auto",
                  config.white_balance_temperature_auto);
  if (config.white_balance_temperature_auto == false) {
    setV4lParameter("white_balance_temperature",
                    config.white_balance_temperature);
  }
}

void updateDynParam(humanoid_robot_camera_setting_tool::CameraParamsConfig &config) {
  g_param_server->updateConfig(config);
}

void setCameraParameterCallback(
    const humanoid_robot_camera_setting_tool::V4lParameter::ConstPtr &msg) {
...
void setCameraParametersCallback(
    const humanoid_robot_camera_setting_tool::V4lParameters::ConstPtr &msg) {
  for (int ix = 0; ix < msg->video_parameter.size(); ix++)
    setV4lParameter(msg->video_parameter[ix].name,
                    msg->video_parameter[ix].value);
...
void setV4lParameter(const std::string &param, const std::string &value) {
  // build the command
  std::stringstream ss;
  ss << "v4l2-ctl --device=" << g_device_name << " -c " << param << "=" << value
     << " 2>&1";
...
bool setParamCallback(humanoid_robot_camera_setting_tool::SetParameters::Request &req,
                      humanoid_robot_camera_setting_tool::SetParameters::Response &res) {
  ...
  // white balance
  setV4lParameter("white_balance_temperature_auto",
                  req.params.white_balance_temperature_auto);
  if (req.params.white_balance_temperature_auto == false) {
    setV4lParameter("white_balance_temperature",
                    req.params.white_balance_temperature);
  }
  ...
  g_dyn_config.white_balance_temperature_auto =
      req.params.white_balance_temperature_auto;
...
bool getParamCallback(humanoid_robot_camera_setting_tool::GetParameters::Request &req,
                      humanoid_robot_camera_setting_tool::GetParameters::Response &res) {
  ...
  res.returns.white_balance_temperature_auto =
      g_dyn_config.white_balance_temperature_auto;
  res.returns.white_balance_temperature =
      g_dyn_config.white_balance_temperature;
...
void resetParameter() {
  ...
  try {
    ...
    g_dyn_config.white_balance_temperature_auto =
        doc["white_balance_temperature_auto"].as<bool>();
    g_dyn_config.white_balance_temperature =
        doc["white_balance_temperature"].as<int>();

  } catch (const std::exception &e) {
    ROS_ERROR_STREAM(
        "Failed to Get default parameters : " << g_default_setting_path);
    return;
  }
...
void publishParam() {
  humanoid_robot_camera_setting_tool::CameraParams cam_params;
  ...
  cam_params.white_balance_temperature_auto =
      g_dyn_config.white_balance_temperature_auto;
...
void saveParameter() {
  ...
  yaml_out << YAML::Key << "brightness" << YAML::Value
           << g_dyn_config.brightness;
  yaml_out << YAML::Key << "contrast" << YAML::Value << g_dyn_config.contrast;
  yaml_out << YAML::Key << "saturation" << YAML::Value
           << g_dyn_config.saturation;
  yaml_out << YAML::Key << "sharpness" << YAML::Value << g_dyn_config.sharpness;
  yaml_out << YAML::Key << "gain" << YAML::Value << g_dyn_config.gain;
  yaml_out << YAML::Key << "focus_auto" << YAML::Value
           << g_dyn_config.focus_auto;
  yaml_out << YAML::Key << "focus_absolute" << YAML::Value
           << g_dyn_config.focus_absolute;
  yaml_out << YAML::Key << "exposure_auto" << YAML::Value
           << g_dyn_config.exposure_auto;
  yaml_out << YAML::Key << "exposure_absolute" << YAML::Value
           << g_dyn_config.exposure_absolute;
  yaml_out << YAML::Key << "white_balance_temperature_auto" << YAML::Value
           << g_dyn_config.white_balance_temperature_auto;
  yaml_out << YAML::Key << "white_balance_temperature" << YAML::Value
           << g_dyn_config.white_balance_temperature;
