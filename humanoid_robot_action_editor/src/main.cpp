...
bool turnOnDynamixelPower(const std::string &device_name,
                          const int &baud_rate) {
  // power on
  dynamixel::PortHandler *_port_h =
  ...
  dynamixel::PacketHandler *_packet_h =
      dynamixel::PacketHandler::getPacketHandler(PROTOCOL_VERSION);

  int _return = _packet_h->write1ByteTxRx(_port_h, SUB_CONTROLLER_ID,
                                          POWER_CTRL_TABLE, 1);
...
int main(int argc, char **argv) {
  ...
  std::string _device_name =
      nh.param<std::string>("device_name", SUB_CONTROLLER_DEVICE);
