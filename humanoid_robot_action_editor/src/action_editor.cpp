...
void ActionEditor::resetSTDin() { tcsetattr(0, TCSANOW, &oldterm); }
...
void ActionEditor::moveUpCursor() {
  if (curr_col_ >= stp7_col_ && curr_col_ <= ccwslope_col_) {
    if (curr_row_ > first_joint_row_) {
      // If last_joint_row_ is less than 9 and the difference from pause_row_ is
      // not 1, it must be specified separately if it is pause_row_.
...
void ActionEditor::moveDownCursor() {
  if (curr_col_ >= stp7_col_ && curr_col_ <= stp6_col_) {
    if (curr_row_ < time_row_) {
      // If last_joint_row_ is less than 9 and the difference from pause_row_ is
      // not 1, it must be specified separately if it is last_joint_row_.
...
bool ActionEditor::initializeActionEditor(std::string robot_file_path,
  ...
  // joint_id_to_row_index_ should be initialized after initialization of
  // joint_id_to_name_.
  int row_idx = 0;
   ...
   */
  for (std::map<std::string, dynamixel::PortHandler *>::iterator it =
           robot_->ports_.begin();
       it != robot_->ports_.end(); it++) {
    port_to_sync_write_go_cmd_[it->first] = new dynamixel::GroupSyncWrite(
        it->second, dynamixel::PacketHandler::getPacketHandler(2.0),
        profile_velocity_addr_, // "profile_velocity"
        8);
  }

  default_editor_script_path_ =
      ros::package::getPath(ROS_PACKAGE_NAME) + "/script/editor_script.yaml";
  mirror_joint_file_path_ = ros::package::getPath(ROS_PACKAGE_NAME) +
                            "/config/config_mirror_joint.yaml";
...
int ActionEditor::convertPositionValueTo4095(int id, int PositionValue) {
  double rad =
      robot_->dxls_[joint_id_to_name_[id]]->convertValue2Radian(PositionValue);
...
bool ActionEditor::loadMirrorJoint() {
  ...
  for (YAML::iterator yaml_it = sub_node2.begin(); yaml_it != sub_node2.end();
       ++yaml_it) {
    int right_id = yaml_it->first.as<int>();
    int left_id = yaml_it->second.as<int>();
...
void ActionEditor::drawPage() {
  ...
  for (std::map<int, std::string>::iterator it = joint_id_to_name_.begin();
    ...
    printf("(");
    for (int joint_name_idx = 0; joint_name_idx < max_joint_name_length_;
         joint_name_idx++) {
    ...
    printf("[    ]");
    goToCursor(cwslope_col_, curr_row_);
    printf("%.1d%.1d", page_.header.pgain[id] >> 4,
           page_.header.pgain[id] & 0x0f);
...
void ActionEditor::readStep() {
  ...
  for (std::map<int, int>::iterator it = joint_id_to_row_index_.begin();
    ...
    if (dxls_it != robot_->dxls_.end()) {
      if (ctrl_->readCtrlItem(joint_name, "torque_enable", &value, &error) ==
          COMM_SUCCESS) {
        if (value == 1) {
          if (ctrl_->readCtrlItem(joint_name, "present_position", &value,
                                  &error) == COMM_SUCCESS) {
            int offset =
                robot_->dxls_[joint_name]->convertRadian2Value(
...
void ActionEditor::clearCmd() { printCmd("                              "); }
...
void ActionEditor::setValueUpDown(int offset) { setValue(getValue() + offset); }
    ...
    if (row == pause_row_) {
      ...
      if (value >= 0 && value <= 4095) {
        ...
        if (!(step_.position[id] & action_file_define::INVALID_BIT_MASK) &&
          ...
          if (ctrl_->writeCtrlItem(joint_id_to_name_[id], "goal_position",
                                   ivalue, &error) == COMM_SUCCESS) {
          ...
          ctrl_->robot_->dxls_[joint_id_to_name_[id]]
              ->dxl_state_->goal_position_ = rad_position;
        }
      }
    }
...
void ActionEditor::toggleTorque() {
  ...
  if (step_.position[id] & action_file_define::TORQUE_OFF_BIT_MASK) {
    ...
    uint32_t value;
    if (ctrl_->readCtrlItem(joint_name, "present_position", &value, 0) !=
        COMM_SUCCESS)
      return;
    ...
    // set goal position
    robot_->dxls_[joint_name]->dxl_state_->goal_position_ =
        robot_->dxls_[joint_name]->convertValue2Radian(value);
  } else {
...
void ActionEditor::storeValueToCache() { cache_value_ = getValue(); }
...
void ActionEditor::clearCache() { cache_value_ = -1; }
...
void ActionEditor::helpCmd() {
  ...
  printf(" new                Clears data of current page and initializes "
         "page.\n");
  ...
  printf(" playboth [index]   Motion playback of current page with [index] "
         "mp3.\n");
  printf(" g [index]          Motion playback of STP[index].\n");
  printf(" name               Name for current page or changes the name of "
         "current page.\n");
  ...
  printf(" mlr [index]        Mirror the left side value to the right of "
         "STP[index]. \n");
  printf(" mrl [index]        Mirror the right side value to the left of "
         "STP[index]. \n");
  printf(" ms [index]         Switch the values between left and right of "
         "STP[index]. \n");
  printf(" on/off             Turn On/Off torque from ALL actuators.\n");
  printf(" on/off [index1] [index2] ...  \n"
         "                    turns On/Off torque from ID[index1] "
         "ID[index2]...\n");
  printf("\n");
...
void ActionEditor::nextCmd() { pageCmd(page_idx_ + 1); }

void ActionEditor::prevCmd() { pageCmd(page_idx_ - 1); }
...
void ActionEditor::playCmd() { playCmd(-1); }
...
void ActionEditor::listCmd() {
  ...
  while (1) {
      ...
      for (int j = 0; j < 4; j++) {
        ...
        if (ActionModule::getInstance()->loadPage(k, &page) == true) {
          ...
          for (int n = 0; n < action_file_define::MAXNUM_NAME; n++) {
            if ((char)page.header.name[n] >= ' ' &&
                (char)page.header.name[n] <= '~')
    ...
    printf("\nAction Page List (%d/3) - Press key n(Next), b(Prev), q(Quit)",
           index + 1);
    while (1) {
      int ch = _getch();
      if (ch == 'n') {
...
void ActionEditor::mirrorStepCmd(int index, int mirror_type, int target_type) {
  ...
  if (target_type == UpperBody || target_type == AllBody) {
    if (mirror_type == RightToLeft || mirror_type == SwitchEach) {
      for (std::map<int, int>::iterator it =
               upper_body_mirror_joints_rl_.begin();
           it != upper_body_mirror_joints_rl_.end(); it++) {
        int right_id = it->first;
        int left_id = it->second;
        int mirror_value =
            convert4095ToMirror(right_id, before_step.position[right_id]);
    ...
    if (mirror_type == LeftToRight || mirror_type == SwitchEach) {
      for (std::map<int, int>::iterator it =
               upper_body_mirror_joints_lr_.begin();
           it != upper_body_mirror_joints_lr_.end(); it++) {
        int left_id = it->first;
        int right_id = it->second;
        int mirror_value =
            convert4095ToMirror(left_id, before_step.position[left_id]);
  ...
  if (target_type == LowerBody || target_type == AllBody) {
    if (mirror_type == RightToLeft || mirror_type == SwitchEach) {
      for (std::map<int, int>::iterator it =
               lower_body_mirror_joints_rl_.begin();
    ...
    if (mirror_type == LeftToRight || mirror_type == SwitchEach) {
      for (std::map<int, int>::iterator it =
               lower_body_mirror_joints_lr_.begin();
  ...
  if (index >= 0 && index < action_file_define::MAXNUM_STEP) {
    for (int i = index; i < action_file_define::MAXNUM_STEP; i++) {
      if (i == (action_file_define::MAXNUM_STEP - 1)) {
        for (int jointIndex = 0; jointIndex < action_file_define::MAXNUM_JOINTS;
             jointIndex++)
          page_.step[i].position[jointIndex] =
              action_file_define::INVALID_BIT_MASK;
...
void ActionEditor::insertInterpolationStepCmd(int index, int ratio) {
  ...
  for (std::map<int, int>::iterator it = joint_id_to_row_index_.begin();
       it != joint_id_to_row_index_.end(); it++) {
    int id = it->first;
    step_a.position[id] = step_a.position[id] * (1 - ratio_double) +
                          step_b.position[id] * ratio_double;
  }
...
void ActionEditor::goCmd(int index) {
  ...
  for (std::map<std::string, dynamixel::GroupSyncWrite *>::iterator it =
           port_to_sync_write_go_cmd_.begin();
  ...
  for (std::map<int, int>::iterator it = joint_id_to_row_index_.begin();
    ...
    if (ctrl_->readCtrlItem(joint_name, "present_position",
                            (uint32_t *)&start_position, 0) != COMM_SUCCESS) {
      printCmd("Failed to read position");
      return;
    }

    goal_position =
        convert4095ToPositionValue(id, page_.step[index].position[id]);
    ...
    // set goal position
    robot_->dxls_[joint_name]->dxl_state_->goal_position_ =
        convert4095ToRadPosition(id, page_.step[index].position[id]);
  }
